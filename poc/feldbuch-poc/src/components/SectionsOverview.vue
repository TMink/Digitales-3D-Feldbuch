<template>
  <div id="wrapper">
      ...
      <v-form>
          <v-list>
              <v-list-item class="mt-5" v-if="current_section.id !== undefined" v-on:click="modifySection(current_section.id)">
                    <v-chip>
                      Derzeit ausgewählt
                    </v-chip>      
                    <v-list-item-content>
              ...
              <v-divider></v-divider>
              <v-subheader v-if="sections.length === 0 && current_section.id == undefined  && loading===false"> Bisher wurden dem Projekt keine Schnitte
